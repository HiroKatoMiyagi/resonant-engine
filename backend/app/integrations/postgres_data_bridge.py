...
from app.services.intent.concurrency import ConcurrencyConfig
...
from app.services.shared.errors import DeadlockError, LockTimeoutError, is_deadlock_error
...
class PostgresDataBridge(DataBridge):
    ...
    async def _persist_locked_intent(self, conn: Connection, intent: Any) -> None:
        await conn.execute(
            """
            UPDATE intents
            SET data = $2::jsonb,
    ...
    @staticmethod
    def _format_intent(row: Record, corrections: Optional[List[Dict[str, Any]]] = None) -> Any:
        # Import here to avoid circular dependency
        from app.models.intent import IntentModel
